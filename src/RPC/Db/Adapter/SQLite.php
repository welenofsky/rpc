...
class SQLite extends MySQL
{
	 ...
	 */
	public function connect( $username = null, $password = null, $options = null )
	{ 
		
		if( ! isset( $GLOBALS['dbconnection'] ) )
		{
			if( $this->_rpc_database )
			{
				if ( ! file_exists( $this->_rpc_database ) ) {
					throw new \Exception('Database "' . $this->_rpc_database . '" not found.');
				}
				$dsn = 'sqlite:' . $this->_rpc_database;
			}
			else
			{
				$dsn = 'sqlite::memory:';
			...
			$GLOBALS['dbconnection'] = new \PDO( $dsn );
		}
	...
	{
		$sql = 'select last_insert_rowid() as n';
		$res = $this->query( $sql );
